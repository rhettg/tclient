...
for f in $(grep -lr tclient $PROJECT_PATH); do
    sed -e "s/tclient/${PROJECT_NAME}/g" < $f > $f.tmp && mv $f.tmp $f
done

# replace all references to Tclient in project files
for f in $(grep -lr Tclient $PROJECT_PATH); do
    sed -e "s/Tclient/${PROJECT_NAME_PROPER}/g" < $f > $f.tmp && mv $f.tmp $f
done

# rename project module directory
mv ${PROJECT_PATH}/tclient ${PROJECT_PATH}/${PROJECT_NAME}

# echo blurb
echo "tclienting of $PROJECT_NAME to $PROJECT_PATH was successful."
